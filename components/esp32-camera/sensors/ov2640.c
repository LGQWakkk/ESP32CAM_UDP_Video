...
int ov2640_init(sensor_t *sensor)
{
    ...
    sensor->set_contrast  = set_contrast;//设置对比度
    sensor->set_brightness= set_brightness;//设置亮度
    sensor->set_saturation= set_saturation;//设置饱和度

    sensor->set_quality = set_quality;
    sensor->set_colorbar = set_colorbar;

    sensor->set_gainceiling = set_gainceiling_sensor;//AGC增益上限
    sensor->set_gain_ctrl = set_agc_sensor;//AGC使能
    sensor->set_exposure_ctrl = set_aec_sensor;//AEC使能
    sensor->set_hmirror = set_hmirror_sensor;
    sensor->set_vflip = set_vflip_sensor;

    sensor->set_whitebal = set_awb_dsp;//AWB
    sensor->set_aec2 = set_aec2;
    sensor->set_aec_value = set_aec_value;//设置曝光时间
